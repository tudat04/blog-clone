...
export default function Register(prop){
    ...
    function register(){
        ...
        axios.get(URl_USER).then((res) => {
            let data = res.data.some((val) => val.username === user.username);
            if(data){
                (alert('trùng tên tài khoản'))
            }else{
                axios.post(URl_USER, user).then((res) => {
            ...
            <CssVarsProvider {...prop}>
                ...
                >
                    ...
                    <FormControl>
                        ...
                        <Input
                            type="text" placeholder="Tên đăng nhập" value={user.name} name="name" onChange={handleChange}
                            sx={{height: 40, display: 'flex', alignItems: 'center', '& input': {paddingY: '0 !important', margin: 0,}}}
                        />
                    </FormControl>
                    <FormControl>
                        <FormLabel>Username</FormLabel>
                        <Input type="text" placeholder="Tên đăng nhập" value={user.username} name="username" onChange={handleChange}
                            sx={{height: 40, display: 'flex', alignItems: 'center', '& input': {paddingY: '0 !important', margin: 0,}}}
                        />
                    </FormControl>
                    <FormControl>
                        <FormLabel>Password</FormLabel>
                        <Input type="password" placeholder="Mật khẩu" value={user.password} name="password" onChange={handleChange}
                            sx={{height: 40, display: 'flex', alignItems: 'center', '& input': {paddingY: '0 !important', margin: 0,}}}
                        />
                    </FormControl>
